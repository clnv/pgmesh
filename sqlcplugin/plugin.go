...
type Options struct {
	...
	// TypeName is the internal combined read-write wrapper type name.
	...
	// ReadInterfaceName is the internal read-only interface name.
	ReadInterfaceName string `json:"read_interface"`
	// WriteInterfaceName is the internal write interface name.
	...
	// ReadTypeName is the internal read-only wrapper type name.
	ReadTypeName string `json:"read_type"`
	// WriteTypeName is the internal primary-capable wrapper type name.
	...
	// NodeConstructorName is the internal database node constructor name.
	...
	// ShardedTypeName is the internal mesh-backed store type name.
...
func parseOptions(req *plugin.GenerateRequest) (*Options, error) {
	...
	if opts.TypeName == "" {
		opts.TypeName = defaultStoreType
	}
	...
	if opts.ReadConstructorName == "" {
		opts.ReadConstructorName = defaultReadNew
	}
	if opts.WriteConstructorName == "" {
		opts.WriteConstructorName = defaultWriteNew
	}
	...
	if opts.ShardedTypeName == "" {
		opts.ShardedTypeName = defaultMeshStoreType
	}
...
func writeStoreConfiguration(out *bytes.Buffer, opts *Options, queries []generatedQuery) {
	...
	out.WriteString("}\n\n")
	fmt.Fprintf(out, "var _ %s = (*%s[uint8])(nil)\n\n", opts.StoreInterfaceName, opts.ShardedTypeName)
