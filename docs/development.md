...
The checked-in same-package, separate-package, and unsharded command-shape
integration fixtures are generated by sqlc v1.31.1 and compiled as part of the
module tests.
...
`just integration-test` runs `go test -p=1 -race -count=1 -tags=integration
./integration/...`. Packages run serially because the fixture packages share
the same isolated database topology.
